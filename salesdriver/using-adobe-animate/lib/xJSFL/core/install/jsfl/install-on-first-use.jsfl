...
// xJSFL Installation file
	...
	function install(window)
	{
		...
		// set up
			...
			// determine xJSFL URI
				xjsflAlreadInstalled = 'xjsfl' in window; 
				xjsfl			= window['xjsfl'] || {};
				xjsfl.uri		= fl.scriptURI.replace('core/install/jsfl/install-on-first-use.jsfl', '');
				xjsfl.loading	= true;
		...
		// splash
			...
			var xul = XUL
					.factory()
					.addLabel('Nessa primeira execução as dependências foram intaladas.')
					.addLabel('Você precisa fechar e reabrir o Animate.')
					.addLabel('Infelizmente agente não pode fazer isso por você.')
					.addLabel('Nas próximas execuções esse passo não será necessário.')
					.setButtons('accept')
					.show();
	}
