	...
	function install(window)
	{
		...
		// checks
			...
			// check that xJSFL is being installed into a folder called xJSFL (not necessary, but useful)
				if(! /\/xJSFL\/core\/install\/jsfl\/no-install.jsfl$/.test(fl.scriptURI))
				{
		...
		// set up
			...
			// determine xJSFL URI
				xjsfl			= window['xjsfl'] || {};
				xjsfl.uri		= fl.scriptURI.replace('core/install/jsfl/no-install.jsfl', '');
				xjsfl.loading	= true;
			...
			// load proxy classes
				...
				fl.trace(xjsfl.uri + 'core/jsfl/proxies.jsfl');
